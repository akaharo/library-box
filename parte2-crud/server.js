...
app.use("/parte1", express.static(assetPath));
...
[
  ...
  app.get(route, (_req, res) => {
    res.sendFile(path.join(assetPath, route.slice(1)));
  });
});
...
async function iniciarBanco() {
  ...
  await inserirSeNaoExistir("books", "title", "Ciencia em Casa", `
    INSERT INTO books (title, author, category, status)
    VALUES ('Ciencia em Casa', 'Rafael Nunes', 'Estudo', 'Na fila')
  `);
...
function validarLivro(dados) {
  ...
  if (!status || status.length > 30) return { erro: "Informe uma situacao com ate 30 caracteres." };
...
iniciarBanco()
  .then(() => {
    app.listen(port, () => {
      console.log(`Library Box CRUD rodando em http://localhost:${port}`);
    });
  })
  .catch((error) => {
